...
Este es un README introductorio, se recomienda ir a la wiki del repositorio para documentación más detallada de cada componente. [Wiki](https://github.com/SebasGirald1003/Entrega3-DS/wiki)
...
## Zona Raw  
Recibe todos los datos en su formato original. Aquí se almacenan tanto los archivos que provienen de la API pública de Covid como las exportaciones generadas desde la base de datos relacional PostgreSQL alojada en AWS RDS.
...
## Zona Refined  
Almacena los resultados finales del proceso analítico en formato CSV. Esta zona permite que los indicadores calculados puedan ser consultados desde Athena o bien expuestos mediante una API.

El flujo completo está automatizado mediante Lambdas, Glue Jobs, Steps de EMR y scripts en AWS Cloudshell. Esto garantiza que todo el proceso, desde la captura hasta la exposición de datos, pueda ser ejecutado de forma programática.
...
La captura de datos del Ministerio de Salud se realiza a través de un servicio Lambda. Este servicio consulta al ejecutarse el endpoint público de Datos Abiertos y almacena los registros obtenidos en la carpeta correspondiente dentro del bucket S3 en la zona Raw.
...
El resultado de este paso se almacena en formato CSV en la zona Trusted del bucket S3, optimizando así las operaciones posteriores de consulta y análisis.
...
El segundo Step del clúster EMR está dedicado a la generación de indicadores. Para ello se emplean DataFrames y consultas SparkSQL, siguiendo las metodologías vistas en el laboratorio 3-4.

Entre los análisis generados se encuentran agregaciones por municipio, por departamento, por fecha y cálculos de indicadores como tasas de mortalidad y recuperación, así como rankings de localidades con mayor impacto.

Los resultados se escriben en la zona Refined de S3 en formatos adecuados para análisis posteriores y para consulta externa utilizando Athena y la Lambda URL.
